...
interface AuthFormProps {
  ...
  /**
   * `h1` on the dedicated sign-in page, `h2` inside the dialog, where the page
   * already owns the h1.
   */
  headingLevel?: "h1" | "h2";
  /** Lets the dialog close itself once a session exists. */
  onSignedIn?: () => void;
  /** Wires the dialog's title to its `aria-labelledby`. */
  titleId?: string;
}

/**
 * Sign in / sign up / forgot password. Shared by the `/signin` page and the
 * app-wide dialog so the two can't drift.
 */
export function AuthForm({
  ...
  function submit(event: React.FormEvent) {
    ...
    signIn({ email, name: mode === "signup" ? name : undefined });
    onSignedIn?.();
  }
  ...
  if (mode === "forgot") {
    return (
      <>
        ...
        <Heading id={titleId} className="font-display mb-2 text-[26px]">
          Forgot password?
        </Heading>
  ...
  return (
    <>
      ...
      <Heading id={titleId} className="font-display mb-1.5 text-[26px]">
        {copy.title}
      </Heading>
      ...
      {/* The original form said plainly that auth wasn't connected. It still
          isn't — this keeps a session in this browser only, and saying so is
          better than implying an account exists on a server somewhere. */}
      <p className="text-fg-subtle mt-5 text-center text-[11.5px] leading-relaxed">
        Demo sign-in: no account is created and nothing is sent anywhere. The
        session is kept in this browser until you log out.
      </p>
    </>
  );
}
